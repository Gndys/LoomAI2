  ...
  return (
    <FeatureCard className={panelClass}>
      <CardContent className="pt-6 space-y-4">
        <VariantHeader
          title="版本 B：轻量文本 + 开关"
          description="弱化容器，让开关更轻盈但依然在一行内。"
        />
  ...
  return (
    <FeatureCard className={panelClass}>
      <CardContent className="pt-6 space-y-4">
        <VariantHeader
          title="版本 C：右侧控制条"
          description="参数左对齐，高级开关靠右，视觉更干净。"
        />
  ...
  return (
    <FeatureCard className={panelClass}>
      <CardContent className="pt-6 space-y-4">
        <VariantHeader
          title="版本 D：标签栏切换"
          description="用强调色边框包裹开关，强化可见性。"
        />
        <div className="flex flex-wrap items-center gap-3">
          <ChipRow />
          <div className="inline-flex items-center gap-2 rounded-md border border-border/60 bg-background/70 px-3 py-1 text-xs text-foreground/80 shadow-sm">
            <span>高级设置</span>
            <Switch checked={open} onCheckedChange={setOpen} />
          </div>
        </div>
...
export default function ImageGeneratePage() {
  return (
    <div className="min-h-screen bg-background text-foreground">
      <FeaturePageShell
        title="AI 图片测试生成页 · 高级设置版本"
        description="仅展示高级设置开关的不同方案，选择一个你喜欢的样式。"
